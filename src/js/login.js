  ...
  addEvent() {
    ...
    $user.blur(function () {
      ...
      if (/^\w{6,10}$/.test($(this).val())) {
        ...
        // 弹出提示框
        cocoMessage.error("请输入正确的账号", 2000);
        // 清除用户名框的值
        $(this).val("");
        // 用户名框未通过验证
        flag = false;
    ...
    $pwd.blur(function () {
      ...
      if (/^\w{8,12}$/.test($(this).val())) {
        ...
        // 否则弹出提示框
        cocoMessage.error("请输入正确的密码", 2000);
        // 清除密码框框的值
        $(this).val("");
        // 密码框未通过验证
        flag = false;
    ...
    this.$login.click(function () {
      ...
      if (flag) {
        ...
        let pwd = $pwd.val();
        // 判断账号是否存在
        $.post('../api/php/query.php', { uname: `${user}` }, function (data){
            // 没有用户名
            if (data=='null') {
                cocoMessage.error("无此用户名，请注册", 0);
            }else{
                // 把账号密码发送给后端
                $.post("../api/php/login.php",{ uname: `${user}`, pwd: `${pwd}` },function (data) {
                    // 判断密码是否正确
                    if (data=='null') {
